...
class AGUC2(object):
    ...
    def move(self,d1,d2):
        ...
        self.axis[self.aliases[0]].move(d1)
        self.axis[self.aliases[0]].amIstill(100)
        self.axis[self.aliases[1]].move(d2)
    ...
    def goToZero(self):
        ...
        self.axis[self.aliases[0]].move(-1*steps1)
        self.axis[self.aliases[0]].amIstill(150)
        self.axis[self.aliases[1]].move(-1*steps2)
...
class AGUC8(object):
    ...
    def __init__(self,portName,activeChannels = ['1'], axis1alias = 'X', axis2alias = 'Y', stepAmp1 = 50, stepAmp2 = 50):
        ...
        # dont wanna use these channels or axes, we use devices instead
        self.aliases = [axis1alias,axis2alias]
        
        self.defChannel = activeChannels[0] #??
        self.currentChannel = activeChannels[0]
    ...
    def chchch(self,ch):
        ...
        channel = int(self.port.sendString('CC?')[2:])
        if channel != ch:
    ...
    def move(self,d1,d2,ch='def'):
        ...
        self.channels[ch][self.aliases[0]].move(d1)
        self.channels[ch][self.aliases[0]].amIstill(100)
        self.channels[ch][self.aliases[1]].move(d2)
    ...
    def goToZero(self,ch='def'):
        ...
        self.channels[ch][self.aliases[0]].move(-1*steps1)
        self.channels[ch][self.aliases[0]].amIstill(150)
        self.channels[ch][self.aliases[1]].move(-1*steps2)
...
'''
#For testing
sss=serial.Serial('COM5',921600,serial.EIGHTBITS,serial.PARITY_NONE,serial.STOPBITS_ONE,timeout=1)
endChar = '\r\n'
def encodeMessage(message):
    return (str(message) + endChar).encode()
sss.write(encodeMessage('MR'))
sss.write(encodeMessage('1PR1000'))
sss.in_waiting
sss.out_waiting
sss.write(encodeMessage('2PR-5000'))
sss.write(encodeMessage('VE'))
sss.readline()
'''